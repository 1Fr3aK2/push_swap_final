...
int	main(int argc, char *argv[])
{
	...
	/*  ft_printf("a_stack antes de ordenar: ");
		r_list *temp_itera = a_stack;
			// Create a separate variable for iteration
		while (temp_itera)
		{
		...
		printf("3 node->number : %d, index: %d\n", a_stack
			->next-> next->number, a_stack->next-> next->index);
		printf("4 node->number : %d, index: %d\n", a_stack
			->next-> next->next->number, a_stack->next-> next->next->index); */
	if (verify(a_stack) == 1)
	{
		if (size == 2)
			sa(&a_stack);
